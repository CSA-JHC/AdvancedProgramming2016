...
# Employee,
# We need a new database interface ASAP. It must be user-friendly (easy to use, no errors, quality instructions, etc). It must be visually appealing while still making since and being compact. I need it to do the following:
...
# This will then need to write the data to a file, and then be able to read that file (all the user needs to do is enter the user id)
...
def setlbl(*args):
    ...
    taxnum.set(("%.2f" % round(taxvalue,2)))
    totalamt = (taxvalue + float(pricentry.get()))# + shipmoney)
    finalprice.set(("%.2f" % round(totalamt,2)))

#finds all the information and sets it to designated areas
#then finds tax and total based on info and sets that
def find(*args):
    ...
    if line[10]==5:
        ship.activate(0)
        ship.selection_anchor(0)
        money=5
    elif line[10]==10:
        ship.activate(1)
        ship.selection_anchor(1)
        money=10
    elif line[10]==15:
        ship.activate(2)
        ship.selection_anchor(2)
        money=15
    elif line[10]==20:
        ship.activate(3)
        ship.selection_anchor(3)
        money=20
    elif line[10]==25:
        ship.activate(4)
        ship.selection_anchor(4)
        money=25       
    ...
    if 0 in shiprice:
        ...
        shipmoney=25
    totalamt=(taxvalue+float(pricentry.get()))#money)
    taxnum.set(("%.2f" % round(taxvalue,2)))
    finalprice.set(("%.2f" % round(totalamt,2)))
...
#finds the shipping cost and adds writes everything to a file, then goes to clear
def taxwrite(*args):
    try:
        ...
        messagebox.showinfo(title='ERROR', message="You didn't enter all the information or you entered something incorrectly.")
        root.mainloop()

#checks to make sure everything is filled in
def check(*args):
    #try:
        if (month.get()=='February' and day.get()=='30') or ((month.get()=='April' or month.get()=='June' or month.get()=='September' or month.get()=='November') and day.get()=='31'):
            messagebox.showinfo(title='ERROR', message="You didn't enter all the information or you entered something incorrectly.")
            root.mainloop()
        elif (useridentry.get() or firstentry.get() or lastentry.get() or streetentry.get() or street2entry.get() or states.get() or cityentry.get() or zipentry.get() or month.get() or day.get() or year.get())=='':
            messagebox.showinfo(title='ERROR', message="You didn't enter all the information or you entered something incorrectly.")
            root.mainloop()
        elif len(useridentry.get())<9 or len(useridentry.get())>9:
            messagebox.showinfo(title='ERROR', message="You didn't enter all the information or you entered something incorrectly.")
            root.mainloop()
        taxwrite(*args)
    #except:
